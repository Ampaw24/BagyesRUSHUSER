  ...
  ResultFuture<List<Category>> getCategories() async {
    ...
    try {
      ...
      return NetworkUtils.handleException(e, s,
          repositoryName: 'OrdersRepository', methodName: 'getCategories');
    }
  }
...
//["Get Orders"]
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final list = _dataList(response)
            ...
            .toList();
        appLogger.i('OrdersRepository.fetchMenuItems → success, loaded ${list.length}');
        return Right(list);
      }
      appLogger.w('OrdersRepository.fetchMenuItems → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
  ...
  ResultFuture<MenuItem> createMenuItem(Map<String, dynamic> data) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        appLogger.i('OrdersRepository.createMenuItem → success, created id=${item.id}');
        return Right(item);
      }
      appLogger.w('OrdersRepository.createMenuItem → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
  ...
  ResultFuture<MenuItem> updateMenuItem({
    ...
    try {
      final body = await _buildMenuItemBody(data);
      final response = await _client.put(ApiEndpoints.vendorMenuItem(id), data: body);
      if (response.statusCode == 200 || response.statusCode == 201) {
        final item = MenuItem.fromJson(_dataMap(response));
        appLogger.i('OrdersRepository.updateMenuItem → success, updated id=${item.id}');
        return Right(item);
      }
      appLogger.w('OrdersRepository.updateMenuItem → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
  ...
  ResultFuture<bool> deleteMenuItem({required String id}) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        appLogger.i('OrdersRepository.deleteMenuItem → success, deleted id=$id');
        return const Right(true);
      }
      appLogger.w('OrdersRepository.deleteMenuItem → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
  ...
  ResultFuture<MenuItem> toggleMenuItemAvailability({
    ...
    appLogger.d('OrdersRepository.toggleMenuItemAvailability → id=$itemId, isAvailable=$isAvailable');
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final item = MenuItem.fromJson(_dataMap(response));
        appLogger.i('OrdersRepository.toggleMenuItemAvailability → success, id=${item.id}');
        return Right(item);
      }
      appLogger.w('OrdersRepository.toggleMenuItemAvailability → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
      appLogger.e('OrdersRepository.toggleMenuItemAvailability → DioException', error: e);
      return NetworkUtils.handleDioException(e);
    } catch (e, s) {
      return NetworkUtils.handleException(
        e,
  ...
  ResultFuture<MenuItem> toggleMenuItemPopular({
    ...
    appLogger.d('OrdersRepository.toggleMenuItemPopular → id=$itemId, isPopular=$isPopular');
    try {
      final response = await _client.patch(
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final item = MenuItem.fromJson(_dataMap(response));
        appLogger.i('OrdersRepository.toggleMenuItemPopular → success, id=${item.id}');
        return Right(item);
      }
      appLogger.w('OrdersRepository.toggleMenuItemPopular → HTTP ${response.statusCode}');
      return NetworkUtils.handleDioResponseError(response);
    } on DioException catch (e) {
      appLogger.e('OrdersRepository.toggleMenuItemPopular → DioException', error: e);
      return NetworkUtils.handleDioException(e);
    } catch (e, s) {
      return NetworkUtils.handleException(
        e,
  ...
  Future<DataMap> _buildMenuItemBody(DataMap data) async {
    ...
    if (localPath != null) {
      ...
      body['image_base64'] = data['image_base64'];
    }
