...
class DashboardViewModel extends ViewModel<DashboardState> {
  ...
  Future<void> toggleStore(bool isOpen) async {
    ...
    await result.fold(
      (failure) async {
        // Revert on failure
        emit(state.copyWith(storeOpen: previous, errorMessage: failure.message));
      },
      (serverIsOpen) async {
        if (serverIsOpen == isOpen) {
          emit(state.copyWith(storeOpen: serverIsOpen, errorMessage: null));
          return;
        }
        ...
        retry.fold(
          ...
          (correctedIsOpen) => emit(state.copyWith(
