...
import 'package:bagyesrushappusernew/src/report/viewmodel/report_form_viewmodel.dart';
import 'package:bagyesrushappusernew/src/report/viewmodel/report_vendors_viewmodel.dart';
...
import 'package:bagyesrushappusernew/src/report/widgets/steps/report_details_step.dart';
import 'package:bagyesrushappusernew/src/report/widgets/steps/report_reason_step.dart';
import 'package:bagyesrushappusernew/src/report/widgets/steps/report_target_picker_step.dart';
import 'package:bagyesrushappusernew/src/report/widgets/steps/report_target_type_step.dart';
...
class ReportFlowView extends StatefulWidget {
  final ReportFlowArgs args;

  const ReportFlowView({super.key, required this.args});

  @override
  State<ReportFlowView> createState() => _ReportFlowViewState();
}

class _ReportFlowViewState extends State<ReportFlowView> {
  late final ReportFormViewModel _vm;
  late ReportFormState _previousState;
  bool _isPickingPhotos = false;

  @override
  void initState() {
    super.initState();
    _vm = sl<ReportFormViewModel>(param1: widget.args);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.args.role == ReportRole.customer) {
      final ordersState =
          context.watch<consumer_orders.OrdersViewModel>().state;
...
class _VendorTargetPicker extends StatefulWidget {
  final _SelectTarget onSelect;

  const _VendorTargetPicker({required this.onSelect});

  @override
  State<_VendorTargetPicker> createState() => _VendorTargetPickerState();
}

class _VendorTargetPickerState extends State<_VendorTargetPicker> {
  late final ReportVendorsViewModel _vm;

  @override
  void initState() {
    super.initState();
    _vm = sl<ReportVendorsViewModel>();
  ...
  Widget build(BuildContext context) {
    ...
    return switch (_vm.state) {
      ReportVendorsLoading() =>
        const Center(child: CircularProgressIndicator(color: AppColors.primary)),
      ReportVendorsError() => Center(
        child: Padding(
      ...
      ReportVendorsLoaded(:final restaurants) => ReportTargetPickerStep(
